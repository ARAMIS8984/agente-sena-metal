    ...
    try:
        ...
        for row in rows[:15]:
            for i, cell in enumerate(row):
                if cell and 'ficha' in str(cell).lower():
                    for c in row:
                        s = str(c).replace('.0','').strip() if c else ''
        ...
        for i, row in enumerate(rows):
            ...
            if any('nombre' in v for v in vals):
                header_row = i
                break
        ...
        col_nombre   = find_col(['nombre'], exclude=['apellido'])
        col_apellido = find_col(['apellido'])
        col_doc      = find_col(['número','documento'], exclude=['tipo']) or find_col(['numero','documento'], exclude=['tipo']) or find_col(['documento'], exclude=['tipo'])
        ...
        for row in rows[header_row+1:]:
            try:
                ...
                if col_estado is not None and len(row) > col_estado:
                    ...
                    estado_raw = str(row[col_estado] or '').strip()
                    estado_norm = ''.join(c for c in _ud.normalize('NFD', estado_raw) if _ud.category(c) != 'Mn').upper()
                ...
                num_doc  = str(row[col_doc] or '').strip() if col_doc is not None and len(row) > col_doc else ''
                num_doc  = num_doc.replace('.0','').strip()
            ...
            if e.get("modo") == "plan" and e.get("paso") == "archivo":
                if fname.lower().endswith(('.xlsx','.xls')):
                    ...
                    try:
                        ...
                        aprendices, ficha = procesar_excel_aprendices(file_bytes, fname)
                        if aprendices:
