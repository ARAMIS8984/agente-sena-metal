...
def plan_detectar_programa(chat_id):
    ...
    if programa_encontrado:
        ...
        set_estado(chat_id, e)
        send_message(chat_id,
            f"✅ Programa detectado: *{programa_encontrado[:60]}*")
        plan_seleccionar_proyecto(chat_id)
    else:
        # No se detectó — mostrar lista para elegir
                ...
                if 'FICHA' in v and not ficha_detectada:
                    for c in row:
                        ...
                        if s.isdigit() and len(s) >= 5:
                            ficha_detectada = s
                            break
                if ('COGIGO' in v or 'CODIGO' in v or 'COGIDO' in v or 'DIGO' in v) and not codigo_programa_detectado:
                    for c in row:
                        s = str(c or '').replace('.0','').strip()
