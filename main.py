...
SYSTEM_ANALISIS = """Eres el Asistente SENA Metalmecánica del Centro Nacional Colombo Alemán, Barranquilla. Eres un colega experimentado, cercano y práctico.

COMPORTAMIENTO CONVERSACIONAL:
- Si el instructor saluda o pregunta cómo estás, responde de forma natural y pregunta en qué puedes ayudar.
- Si describe una situación de un aprendiz, analiza y responde directamente.
- Si no sabe qué falta aplica, guíalo con preguntas simples para identificarla.
- Siempre ofrece dos caminos: guía paso a paso O menú de opciones.
- Si detectas un error en datos previos (nombre mal escrito, ficha incorrecta), corrígelo sin reiniciar.
- Sé breve: máximo 4 líneas por respuesta.

CUANDO IDENTIFIQUES UNA FALTA, responde así:

*Art. [número] — Acuerdo 0009 de 2024:*
"[Texto exacto y breve del artículo]"

📋 Aplica: [tipo de documento]

¿Lo generamos?

ARTÍCULOS EXACTOS DEL ACUERDO 0009 DE 2024:
- Art. 8 lit. d: "El aprendiz debe asistir puntualmente a todas las actividades de formación y justificar debidamente las inasistencias."
- Art. 16: "Tres (3) o más ausencias injustificadas en el trimestre → Primer Llamado de Atención disciplinario."
- Art. 17: "Cinco (5) o más ausencias injustificadas → Segundo Llamado de Atención + Plan de Mejoramiento obligatorio."
- Art. 18: "Más del 20% de inasistencias totales → riesgo de desvinculación del programa de formación."
- Art. 22: "La nota mínima aprobatoria por competencia es 3.0 sobre 5.0."
- Art. 23: "Nota inferior a 3.0 → Plan de Mejoramiento académico obligatorio con fecha límite de cumplimiento."
- Art. 24: "Si no supera el Plan de Mejoramiento → segunda oportunidad mediante Acta Extraordinaria."
- Art. 25: "Nota igual o superior a 4.5 en todas las competencias → Reconocimiento por desempeño destacado."
...
- Art. 8 lit. f: "El aprendiz debe mantener comportamiento respetuoso con toda la comunidad educativa."
- Art. 8 lit. c: "El aprendiz debe presentarse en condiciones físicas y mentales aptas para la formación."
- Art. 20: "Consumo de sustancias psicoactivas → retiro inmediato y cancelación de matrícula."
- Art. 41 num. 2: "Constituyen faltas disciplinarias las conductas que afecten la convivencia y el normal desarrollo de la formación."

Responde en español. Sé directo, cálido y práctico."""

SYSTEM_REGLAMENTO = """Eres experto en el Acuerdo 0009 de 2024 del Reglamento del Aprendiz SENA.

Responde SIEMPRE así — máximo 4 líneas:

*Art. [número] — Acuerdo 0009 de 2024:*
"[Texto exacto y breve del artículo]"

📌 En la práctica: [una línea de aplicación]
...
- Art. 18: "Más del 20% de inasistencias totales → riesgo de desvinculación del programa de formación."
- Art. 22: "La nota mínima aprobatoria por competencia es 3.0 sobre 5.0."
- Art. 23: "Nota inferior a 3.0 → Plan de Mejoramiento académico obligatorio con fecha límite."
- Art. 24: "Si no supera el Plan de Mejoramiento → segunda oportunidad mediante Acta Extraordinaria."
- Art. 25: "Nota igual o superior a 4.5 en todas las competencias → Reconocimiento por desempeño destacado."
- Art. 28: "El aprendiz debe entregar mínimo el 60% de evidencias por competencia."
- Art. 29: "Menos del 60% de evidencias → Plan de Mejoramiento con cronograma de entrega."
- Art. 8 lit. f: "Mantener comportamiento respetuoso con toda la comunidad educativa."
- Art. 8 lit. c: "Presentarse en condiciones físicas y mentales aptas para la formación."
- Art. 20: "Consumo de sustancias psicoactivas → retiro inmediato y cancelación de matrícula."
- Art. 41 num. 2: "Conductas que afecten la convivencia y el normal desarrollo de la formación."
...
def menu_principal(chat_id):
    set_estado(chat_id, {"modo": "conversacion"})
    conversaciones.pop(chat_id, None)
    send_message(chat_id,
        "¡Hola! ¿Cómo va tu día? 👋\n\n"
        "_Cuéntame en qué te ayudo, o si prefieres ir directo a las herramientas:_",
        reply_markup={"inline_keyboard": [
            [{"text": "📋 Ver herramientas", "callback_data": "ver_menu"}],
        ]}
    )

def ver_menu(chat_id):
...
def webhook():
    try:
        ...
        # ── FLUJO SEGUIMIENTO / CONVERSACION (análisis IA) ──
        elif modo in ("seguimiento","seguimiento_menu","conversacion"):
            if "historial" not in e:
                e["historial"] = []
            e["historial"].append({"role":"user","content":text})
            if len(e["historial"]) > 10:
                e["historial"] = e["historial"][-10:]
            set_estado(chat_id, e)
            send_typing(chat_id)
                ...
                send_message(chat_id, respuesta,
                    reply_markup={"inline_keyboard":[
                        [{"text":"📋 Generar Llamado","callback_data":"llamado_inicio"},
                         {"text":"🛠️ Herramientas","callback_data":"ver_menu"}],
                    ]})
            else:
        ...
        # ── MODO CONVERSACIONAL — IA interpreta libremente ──
        else:
            txt = text.lower()
            # Detección rápida de intenciones claras
            if any(p in txt for p in ["llamado de atención", "llamado de atencion", "generar llamado"]):
                send_message(chat_id,
                    "Entendido. ¿Prefieres que te guíe paso a paso hasta terminar el documento, "
                    "o ir directo al menú de opciones?",
                    reply_markup={"inline_keyboard": [
                        [{"text": "🧭 Guíame paso a paso", "callback_data": "llamado_inicio"}],
                        [{"text": "📋 Ver menú de opciones", "callback_data": "ver_menu"}],
                    ]})
            elif any(p in txt for p in ["plan concertado", "concertado"]):
                send_message(chat_id,
                    "Claro. ¿Te guío paso a paso o prefieres ir directo al menú?",
                    reply_markup={"inline_keyboard": [
                        [{"text": "🧭 Guíame paso a paso", "callback_data": "menu_plan"}],
                        [{"text": "📋 Ver menú de opciones", "callback_data": "ver_menu"}],
                    ]})
            elif any(p in txt for p in ["reglamento", "acuerdo", "artículo", "articulo", "norma"]):
                ...
                if r:
                    send_message(chat_id, r,
                        reply_markup={"inline_keyboard":[
                            [{"text":"❓ Otra consulta","callback_data":"menu_reglamento"}],
                            [{"text":"🛠️ Ver herramientas","callback_data":"ver_menu"}],
                        ]})
            else:
                # IA interpreta libremente — saludo, novedad, pregunta general
                if "historial" not in get_estado(chat_id):
                    set_estado(chat_id, {"modo":"conversacion","historial":[]})
                e = get_estado(chat_id)
                if "historial" not in e:
                    e["historial"] = []
                e["historial"].append({"role":"user","content":text})
                ...
                set_estado(chat_id, e)
                send_typing(chat_id)
                r = consultar_groq(e["historial"], SYSTEM_ANALISIS)
                if r:
                    e["historial"].append({"role":"assistant","content":r})
                    set_estado(chat_id, e)
                    send_message(chat_id, r,
                        reply_markup={"inline_keyboard":[
                            [{"text":"📋 Generar Llamado","callback_data":"llamado_inicio"},
